...
template <typename T>
void INTRINSICCV_TARGET_FN_ATTRS add_abs_with_threshold(
  ...
  add_abs_with_threshold_sc(src_a, src_a_stride, src_b, src_b_stride, dst,
                            dst_stride, width, height, threshold);
}

#define INTRINSICCV_INSTANTIATE_TEMPLATE(type)                            \
  template void INTRINSICCV_TARGET_FN_ATTRS add_abs_with_threshold<type>( \
