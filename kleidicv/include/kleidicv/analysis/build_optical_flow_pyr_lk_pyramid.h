...
// Shared parameter validation and actual-level computation used by both
// single-thread and threaded build entry points.
inline kleidicv_error_t validate_and_compute_build_optical_flow_pyr_lk_levels(
  ...
  size_t row_bytes = 0;
  if (__builtin_mul_overflow(width, channels, &row_bytes)) {
    return KLEIDICV_ERROR_RANGE;
  }
...
class OpticalFlowLKPyramid final {
 public:
  ...
  // OpticalFlowLKPyramid storage is only allocatable with allocate().
 ...
 private:
  ...
  static void fill_reflect_101_border_in_place(uint8_t* interior, size_t stride,
    ...
    // Process in "logical image coordinates" where:
    // x in [-border_width, width + border_width)
    // y in [-border_height, height + border_height)
    //
    // Negative / out-of-range coordinates are mapped back into the interior
    // image via reflect-101 and copied into the border region.
    const ptrdiff_t x_begin = -static_cast<ptrdiff_t>(border_width);
    const ptrdiff_t x_end = static_cast<ptrdiff_t>(width + border_width);
    const ptrdiff_t y_begin = -static_cast<ptrdiff_t>(border_height);
    const ptrdiff_t y_end = static_cast<ptrdiff_t>(height + border_height);
    const ptrdiff_t width_end = static_cast<ptrdiff_t>(width);
    const ptrdiff_t height_end = static_cast<ptrdiff_t>(height);

    // Fill top border rows: y in [-border_height, -1].
    for (ptrdiff_t y = y_begin; y < 0; ++y) {
      const size_t src_y = border_index_reflect_101(y, height);
      for (ptrdiff_t x = x_begin; x < x_end; ++x) {
        const size_t src_x = border_index_reflect_101(x, width);
        // Source pixel is inside interior after reflect-101 mapping.
        const uint8_t* src = interior + src_y * stride + src_x * channels;
        // Destination pixel can be in top/left/right border by construction.
        uint8_t* dst = interior + y * static_cast<ptrdiff_t>(stride) +
                       x * static_cast<ptrdiff_t>(channels);
        std::memcpy(dst, src, channels);
      }
    }

    // Fill bottom border rows: y in [height, height + border_height - 1].
    for (ptrdiff_t y = height_end; y < y_end; ++y) {
      const size_t src_y = border_index_reflect_101(y, height);
      for (ptrdiff_t x = x_begin; x < x_end; ++x) {
        const size_t src_x = border_index_reflect_101(x, width);
        const uint8_t* src = interior + src_y * stride + src_x * channels;
    ...
    // Fill side borders for interior rows only:
    // - left border x in [-border_width, -1]
    // - right border x in [width, width + border_width - 1]
    //
    // Top/bottom corners were already written in the two loops above.
    for (ptrdiff_t y = 0; y < height_end; ++y) {
      for (ptrdiff_t x = x_begin; x < 0; ++x) {
        const size_t src_x = border_index_reflect_101(x, width);
        const uint8_t* src = interior + y * stride + src_x * channels;
        uint8_t* dst = interior + y * static_cast<ptrdiff_t>(stride) +
                       x * static_cast<ptrdiff_t>(channels);
        std::memcpy(dst, src, channels);
      }
      for (ptrdiff_t x = width_end; x < x_end; ++x) {
        const size_t src_x = border_index_reflect_101(x, width);
        const uint8_t* src = interior + y * stride + src_x * channels;
        uint8_t* dst = interior + y * static_cast<ptrdiff_t>(stride) +
                       x * static_cast<ptrdiff_t>(channels);
