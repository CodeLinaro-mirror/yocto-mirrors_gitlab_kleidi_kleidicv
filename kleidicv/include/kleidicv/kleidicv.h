...
/// @ref KLEIDICV_MAX_IMAGE_PIXELS.
///
/// Generic downsizing algorithm accuracy:
/// For the best performance, 2-D linear interpolation uses 8-bit weights.
/// Its maximum error from the exact value can be calculated as below.
///
...
///   R = A + ((B-A) * W) / 256
/// The maximum error happens at A=0 and B=255, substituting these the error is
/// the difference between the result (R) and the exact result (Re), plus a
/// rounding error of 1/2:
///   E1D = Re - R = 255 * E / 256 + 1/2 = 511 / 512
///
/// For two dimensions (i.e. doing the horizontal interpolation after the
/// vertical one), the formula is the same, but here A and B are the results of
/// the 1D calculation above, so they also have some error:
///   A = Ae + E1D
///   B = Be + E1D
///   W = We + E
...
///          -> 2 * E1D * We/256  + (Be - Ae)/256 * E
///             (E1D*E is very small, can be ignored)
///   rounding error of 1/2
/// From these, Be - Ae <= 255 and We <= 255, so the total error is:
