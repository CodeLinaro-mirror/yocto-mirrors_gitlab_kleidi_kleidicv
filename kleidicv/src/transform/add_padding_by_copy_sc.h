    ...
    loop.unroll_four_times(copy_block_quad)
        .remaining([&](size_t index, size_t length) KLEIDICV_STREAMING {
          for (; index < length; index += kStep) {
            svbool_t tail_pg = svwhilelt_b8_u64(index, length);
...
class RepeatedPixelWriter : public ContiguousByteCopier {
 ...
 private:
  ...
  static inline void fill_interleaved_3(
    ...
    const Vector3Type vector_value = svcreate3(
        load_repeated_unaligned<ScalarType>(value_bytes),
  ...
  // Load the scalar bytes directly into a vector register and broadcast lane 0.
  template <typename ScalarType>
  static inline typename VecTraits<ScalarType>::VectorType
  load_repeated_unaligned(const uint8_t *src) KLEIDICV_STREAMING {
    if constexpr (sizeof(ScalarType) == sizeof(uint8_t)) {
      const svuint8_t bytes = svld1_u8(svptrue_pat_b8(SV_VL1), src);
      return svdup_lane_u8(bytes, 0);
    } else if constexpr (sizeof(ScalarType) == sizeof(uint16_t)) {
      const svuint8_t bytes = svld1_u8(svptrue_pat_b8(SV_VL2), src);
      return svdup_lane_u16(svreinterpret_u16_u8(bytes), 0);
    } else if constexpr (sizeof(ScalarType) == sizeof(uint32_t)) {
      const svuint8_t bytes = svld1_u8(svptrue_pat_b8(SV_VL4), src);
      return svdup_lane_u32(svreinterpret_u32_u8(bytes), 0);
    } else {
      const svuint8_t bytes = svld1_u8(svptrue_pat_b8(SV_VL8), src);
      return svdup_lane_u64(svreinterpret_u64_u8(bytes), 0);
    }
  }
};
