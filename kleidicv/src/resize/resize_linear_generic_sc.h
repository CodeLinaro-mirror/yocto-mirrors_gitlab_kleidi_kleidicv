...
class ResizeGenericU8Operation final {
 ...
 private:
  ...
  // Adding sx values again and again accumulates error, recalibrating
  // resets it. This is the number of pixels between recalibrations.
  static constexpr ptrdiff_t kRecalibrateStep = 250;
  ...
  kleidicv_error_t allocate_temp_buffers() KLEIDICV_STREAMING {
    ...
    rounded_width_ = align_up(dst_width_, kStep_);
  ...
  svuint32_t make_vsx0(uint64_t dx) const KLEIDICV_STREAMING {
    ...
    svuint64_t sx_b = svmla_n_u64_x(svptrue_b64(), half, svindex_u64(dx, 4),
                                    src_width_ << kFixpBits);
    svuint64_t sx_t = svmla_n_u64_x(svptrue_b64(), half, svindex_u64(dx + 2, 4),
                                    src_width_ << kFixpBits);
    return svtrn1_u32(
  ...
  void calculate_indices_fractions_base(
    ...
    if constexpr (kRatio == 2) {
      svst1(svptrue_b8(), idx_ + dx, vsx_left_idx);
    } else if constexpr (kRatio == 3) {
      svuint8_t vsx_right_idx = svadd_n_u8_x(svptrue_b8(), vsx_left_idx, 1);
      ...
      svst1(svptrue_b8(), idx_ + 2 * dx, vsx_idx_lo);
      svst1_vnum(svptrue_b8(), idx_ + 2 * dx, 1, vsx_idx_hi);
    }
    ...
    svst1(svptrue_b16(), xfrac_ + dx, vsxfrac_b);
    svst1_vnum(svptrue_b16(), xfrac_ + dx, 1, vsxfrac_t);
  }

  void precalculate_indices_fractions_bases() KLEIDICV_STREAMING {
    // Outer loop for recalibrate sx_fixp, the inner loop only iterates
    // using addition, it's faster than multiplying at each step
    size_t i = 0;
    size_t dx = 0;
    size_t max_i = rounded_width_ / kStep_;
    uint64_t max_sx = std::max(src_width_ - kStep_ * kRatio, 0UL);
    while (dx < dst_width_) {
      uint64_t sx_fixp = to_src_x(dx);
      size_t i_end = std::min(max_i, i + kRecalibrateStep);
      while (i < i_end) {
        uint64_t sx_base = sx_fixp >> kFixpBits;
        max_2x_vector_path_ = sx_base <= max_sx ? static_cast<ptrdiff_t>(i) - 1
                                                : max_2x_vector_path_;
  ...
  svuint8_t interpolate(ptrdiff_t dx, uint16_t yfrac, svuint8_t a, svuint8_t b,
                        svuint8_t c, svuint8_t d) const KLEIDICV_STREAMING {
#if KLEIDICV_TARGET_SME2
    svuint16x2_t vsxfrac = svld1_x2(svptrue_c8(), xfrac_ + dx);
    svuint16_t vsxfrac_b = svget2(vsxfrac, 0);
    svuint16_t vsxfrac_t = svget2(vsxfrac, 1);
#else
    svuint16_t vsxfrac_b = svld1(svptrue_b16(), xfrac_ + dx);
    svuint16_t vsxfrac_t = svld1_vnum(svptrue_b16(), xfrac_ + dx, 1);
#endif
  ...
  svuint8_t common_vector_path_r2(
      ptrdiff_t dx, uint16_t yfrac, svuint8x2_t topsrc,
      svuint8x2_t bottomsrc) const KLEIDICV_STREAMING {
    svuint8_t vsx0_idx = svld1(svptrue_b8(), idx_ + dx);
    svuint8_t vsx1_idx = svadd_n_u8_x(svptrue_b8(), vsx0_idx, 1);
    ...
    return interpolate(dx, yfrac, a, b, c, d);
  }

  svuint8x2_t vector_path_2x_r2(ptrdiff_t i, ptrdiff_t dx, uint16_t yfrac,
    ...
    return svcreate2(
        common_vector_path_r2(dx, yfrac, topsrc0, bottomsrc0),
        common_vector_path_r2(dx + kStep_, yfrac, topsrc1, bottomsrc1));
  }

  svuint8_t vector_path_r2(ptrdiff_t i, ptrdiff_t dx, uint16_t yfrac,
                           Columns<const uint8_t> src_cols_top,
                           Columns<const uint8_t> src_cols_bottom) const
      KLEIDICV_STREAMING {
    ...
    svuint8x2_t topsrc =
        load8x2_while_u8(src_cols_top.ptr_at(sx_base),
                         static_cast<uint64_t>(sx_base), src_width_);
    svuint8x2_t bottomsrc =
        load8x2_while_u8(src_cols_bottom.ptr_at(sx_base),
                         static_cast<uint64_t>(sx_base), src_width_);
    return common_vector_path_r2(dx, yfrac, topsrc, bottomsrc);
  }

  svuint8_t common_vector_path_r3(
      ptrdiff_t dx, uint16_t yfrac, svuint8x3_t topsrc,
      svuint8x3_t bottomsrc) const KLEIDICV_STREAMING {
#if KLEIDICV_TARGET_SME2
    svuint8x2_t vidx = svld1_x2(svptrue_c8(), idx_ + 2 * dx);
    svuint8_t vsx_idx0 = svget2(vidx, 0);
    svuint8_t vsx_idx1 = svget2(vidx, 1);
#else
    svuint8_t vsx_idx0 = svld1(svptrue_b8(), idx_ + 2 * dx);
    svuint8_t vsx_idx1 = svld1_vnum(svptrue_b8(), idx_ + 2 * dx, 1);
#endif
    svuint8_t ab0 =
        svtbl2_u8(svcreate2(svget3(topsrc, 0), svget3(topsrc, 1)), vsx_idx0);
    svuint8_t ab1 =
        svtbl2_u8(svcreate2(svget3(topsrc, 1), svget3(topsrc, 2)), vsx_idx1);
    svuint8_t cd0 = svtbl2_u8(
        svcreate2(svget3(bottomsrc, 0), svget3(bottomsrc, 1)), vsx_idx0);
    svuint8_t cd1 = svtbl2_u8(
        svcreate2(svget3(bottomsrc, 1), svget3(bottomsrc, 2)), vsx_idx1);
    svuint8_t a = svuzp1_u8(ab0, ab1);
    svuint8_t b = svuzp2_u8(ab0, ab1);
    svuint8_t c = svuzp1_u8(cd0, cd1);
    svuint8_t d = svuzp2_u8(cd0, cd1);
    return interpolate(dx, yfrac, a, b, c, d);
  }

  svuint8x2_t vector_path_2x_r3(ptrdiff_t i, ptrdiff_t dx, uint16_t yfrac,
                                Columns<const uint8_t> src_cols_top,
                                Columns<const uint8_t> src_cols_bottom) const
      KLEIDICV_STREAMING {
    ptrdiff_t sx_base0 = sxbase_[i], sx_base1 = sxbase_[i + 1];
    ...
    return svcreate2(
        common_vector_path_r3(dx, yfrac, topsrc0, bottomsrc0),
        common_vector_path_r3(dx + kStep_, yfrac, topsrc1, bottomsrc1));
  }

  svuint8_t vector_path_r3(ptrdiff_t i, ptrdiff_t dx, uint16_t yfrac,
                           Columns<const uint8_t> src_cols_top,
                           Columns<const uint8_t> src_cols_bottom) const
      KLEIDICV_STREAMING {
    ptrdiff_t sx_base = sxbase_[i];
    ...
    svuint8x3_t topsrc =
        load8x3_while_u8(src_cols_top.ptr_at(sx_base),
                         static_cast<uint64_t>(sx_base), src_width_);
    svuint8x3_t bottomsrc =
        load8x3_while_u8(src_cols_bottom.ptr_at(sx_base),
                         static_cast<uint64_t>(sx_base), src_width_);
    return common_vector_path_r3(dx, yfrac, topsrc, bottomsrc);
  }
  ...
  void process_row(uint64_t dy) const KLEIDICV_STREAMING {
    ...
    while (i < max_2x_vector_path_) {
      ...
      if constexpr (kRatio == 3) {
        res = vector_path_2x_r3(i, dx, yfrac, src_cols_top, src_cols_bottom);
      } else if constexpr (kRatio == 2) {
        res = vector_path_2x_r2(i, dx, yfrac, src_cols_top, src_cols_bottom);
      }
#if KLEIDICV_TARGET_SME2
      svst1(svptrue_c8(), dst_cols.ptr_at(dx), res);
#else
      svst1(svptrue_b8(), dst_cols.ptr_at(dx), svget2(res, 0));
      svst1_vnum(svptrue_b8(), dst_cols.ptr_at(dx), 1, svget2(res, 1));
    ...
    for (; dx < static_cast<ptrdiff_t>(dst_width_); dx += kStep_) {
      svbool_t pgdst = svwhilelt_b8(dx, static_cast<ptrdiff_t>(dst_width_));
      svuint8_t res;
      if constexpr (kRatio == 2) {
        res = vector_path_r2(i, dx, yfrac, src_cols_top, src_cols_bottom);
      } else if constexpr (kRatio == 3) {
        res = vector_path_r3(i, dx, yfrac, src_cols_top, src_cols_bottom);
      }
      svst1(pgdst, dst_cols.ptr_at(dx), res);
      ++i;
    }
  }
...
kleidicv_error_t kleidicv_resize_generic_stripe_u8_sc(
  ...
  svuint8_t s4, s5;
  ResizeGenericU8Operation<kRatio> operation(
...
#define KLEIDICV_INSTANTIATE_TEMPLATE_SC(ratio)                          \
  template kleidicv_error_t kleidicv_resize_generic_stripe_u8_sc<ratio>( \
...
KLEIDICV_INSTANTIATE_TEMPLATE_SC(2L);
KLEIDICV_INSTANTIATE_TEMPLATE_SC(3L);
