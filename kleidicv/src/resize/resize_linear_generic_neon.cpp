...
class ResizeGenericU8Operation final {
  ...
  ResizeGenericU8Operation(const uint8_t *src, size_t src_stride,
      ...
      : src_rows_{src, src_stride, kChannels},
        ...
        sx_fixp_step_{static_cast<uint64_t>(
            (((src_width_ * kStep / kChannels) << kFixpBits) + dst_width_ / 2) /
            dst_width_)},
 ...
 private:
  ...
  // it. This is the number of elements between recalibrations.
  ...
  static size_t calculate_num_of_vector_path_2x(size_t src_width,
                                                size_t dst_width) {
    if ((src_width * kChannels) >= sizeof(std::conditional_t < kRatio == 2,
                                          uint8x16x2_t, uint8x16x3_t >)) {
  ...
  void precalculate_indices_fractions_bases() {
    ...
    while (i < num_of_vector_path_2x_) {
      ...
      size_t i_end = std::min(num_of_vector_path_2x_, i + kRecalibrateStep);
