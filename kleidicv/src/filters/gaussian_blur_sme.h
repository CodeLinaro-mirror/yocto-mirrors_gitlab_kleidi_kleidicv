...
static kleidicv_error_t gaussian_blur_stripe_u8_sme(
    const uint8_t *src, size_t src_stride, uint8_t *dst, size_t dst_stride,
    ...
    FixedBorderType fixed_border_type,
    kleidicv_filter_context_t *context) KLEIDICV_STREAMING_COMPATIBLE {
  if (!gaussian_blur_sme_implementation_checks(kernel_width, kernel_height,
                                               channels)) {
    return gaussian_blur_stripe_u8_sc(src, src_stride, dst, dst_stride, width,
                                      height, y_begin, y_end, channels,
