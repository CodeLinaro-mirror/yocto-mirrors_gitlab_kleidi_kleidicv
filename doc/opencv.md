...
* For single-channel (CV_8UC1) upscaling in single-threaded mode, KleidiCV is disabled when the
horizontal scaling factor exceeds 1.4. All other supported configurations use KleidiCV.

Note: For the generic linear resize, horizontal scaling factor is limited to 0.33 - infinity, but
vertical scaling factor can be anything.
