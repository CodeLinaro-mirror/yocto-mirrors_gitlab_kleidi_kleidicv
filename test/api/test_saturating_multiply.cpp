...
#define INTRINSICCV_SATURATING_MULTIPLY(type, suffix) \
  INTRINSICCV_API(saturating_multiply,                \
                  intrinsiccv_saturating_multiply_##suffix, type)

INTRINSICCV_SATURATING_MULTIPLY(uint8_t, u8);
INTRINSICCV_SATURATING_MULTIPLY(int8_t, s8);
INTRINSICCV_SATURATING_MULTIPLY(uint16_t, u16);
INTRINSICCV_SATURATING_MULTIPLY(int16_t, s16);
INTRINSICCV_SATURATING_MULTIPLY(int32_t, s32);
...
TYPED_TEST(SaturatingMultiply, ZeroImageSize) {
  TypeParam src[1] = {}, dst[1];
  EXPECT_EQ(INTRINSICCV_OK, saturating_multiply<TypeParam>()(
