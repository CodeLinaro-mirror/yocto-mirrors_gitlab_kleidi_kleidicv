...
cv::Mat exec_separable_filter_2d(cv::Mat& input) {
  uint32_t kernel_seed =
      *reinterpret_cast<uint32_t*>(&input.at<uint8_t>(input.rows - 1, 0));
...
bool test_separable_filter_2d(int index, RecreatedMessageQueue& request_queue,
  ...
  for (size_t y = 5; y <= 16; ++y) {
    for (size_t x = 5; x <= 16; ++x) {
      ...
      // to the real input
      cv::Mat input(y + 1, x, CV_8UC(Channels));
      ...
      *reinterpret_cast<uint32_t*>(&input.at<uint8_t>(input.rows - 1, 0)) =
      ...
      cv::Mat actual = exec_separable_filter_2d<KernelSize, BorderType>(input);
...
std::vector<test>& separable_filter_2d_tests_get() {
  ...
  static std::vector<test> tests = {
    ...
    TEST("Separable Filter 2D 5x5, BORDER_REPLICATE, 3 channel", (test_separable_filter_2d<5, cv::BORDER_REPLICATE, 3>), (exec_separable_filter_2d<5, cv::BORDER_REPLICATE>)),
    TEST("Separable Filter 2D 5x5, BORDER_REPLICATE, 4 channel", (test_separable_filter_2d<5, cv::BORDER_REPLICATE, 4>), (exec_separable_filter_2d<5, cv::BORDER_REPLICATE>)),
  };
  // clang-format on
  return tests;
}
